...
class Playlists(View):
    ...
    def put(self, request):
        request.AUTH['Content-Type'] = 'application/json'
    ...
    def post(self, request):
        ...
        for playlist_id in playlist_ids:
            for i in range(0, len(track_ids), 100):
                ...
                response = requests.post(url=url, headers=request.AUTH)
...
class PlaylistTracks(View):
    ...
    def put(self, request):
        ...
        response = requests.put(url=SPOTIFY_API_URL + '/playlists/' + playlist_id + '/tracks?uris=', headers=request.AUTH)
        if response.status_code != 201:
            return HttpResponse(status=response.status_code)
    ...
    def delete(self, request):
        ...
        for i in range(0, len(track_ids), 100):
            ...
            response = requests.delete(url=url, json=data, headers=request.AUTH)
            if response.status_code != 200:
                return HttpResponse(status=response.status_code)
        return HttpResponse(status=200)
